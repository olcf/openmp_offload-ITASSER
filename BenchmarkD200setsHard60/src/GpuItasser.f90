...
********************** start i-cicle ****************************
*****************************************************************
!$acc loop gang private(i,fact,axki,ayki,azki,dist,bxi,byi,bzi,
...
! ! !$acc parallel loop gang worker vector async(1) reduction(+:ESHORT2)
! ! !$acc&  default(present) 
      do 1 i=iiii,jjjj
         iseq=seq(i)
         if(mv(i).gt.0)then
...
!$acc& ,afs(:),noaa(:),nomm(:),envir(:,:,:,:,:)) !copyout(noa(:))
!$acc kernels async(2) 
! !$acc loop gang(1024)
         do pp=1,Lch
...
c     calculate eprofn while dord was calculated when call EHB(m,m2,1)--->
         eprofn=0.0
!$acc kernels loop gang reduction(+:eprofn) async(2)
         do pp=1,Lch
            is=seq(pp)          
            ia=noa(pp)          
...
c     calculate eprofn while dord was calculated when call EHB(m,m3,1)--->
         eprofn=0.0
!$acc parallel loop gang reduction(+:eprofn)
         do pp=1,Lch
            is=seq(pp)
            ia=noa(pp)
            ip=nop(pp)
...
c     calculate E_new--------------->
!$acc data present(nop(:),nom(:),noa(:),afsn(:),afs(:)
!$acc& ,noaa(:),nopp(:),nomm(:))
!$acc kernels
         do pp=1,Lch
            nop(pp)=nopp(pp)    !prepare to calculate energy
...
!$acc kernels
         ...
         enddo
!$acc end kernels
! !$acc update device(ex,ey,ez,egx,egy,egz,ecx,ecy,ecz,ebx,
! !$acc&   eby,ebz,etx,ety,etz,x,y,z)
