...
!!!!!!!!!!!!!! This One !!!!!!!!!!!!!!!!!!!!!!!
! !$OMP target teams distribute parallel do nowait       !!!This is correct 1
!!!!!!!!!!!!!!!!!!!1 target data  map region start 
! !$OMP target data map(to:sec,jbin,seq,x,z,y,hbz,ez,gx,gy,gz,
...
! !$OMP target teams loop  ! reduction(+:EHB1,EHB1a,EHB1b,EHB1c,EHB2,EHB3,EHB4,
! !$OMP target data map(to:sec,jbin,seq,x,z,y,hbz,ez,gx,gy,gz,
! !$OMP&  hbx,hby,arlp,cax,cay,egz,ex,ey,ecz,egx,egy,concut2,ebx,
! !$OMP& eby,ebz,ecx,ecy,ica,caz,apa,alm,nom,arlm,apm,acops,mv,
! !$OMP& app,alp,ala,noa,arla,apar,nop,ehb1)
! !$OMP&  map(to:ehb2,ehb5b,sumct1,icnt1,ehb1c,
! !$OMP& ehb1a,ehb1b,ehb5a,ehb4,ehb3)
! !$OMP&                        EHB5,EHB5a,EHB5b,ICNT1,SUMCT1)
! !$OMP target teams distribute
!!!!!!!!!!!!!!! correct target enter data 
!NEW1  !$OMP target  enter data map(to:sec,jbin,seq,x,z,y,hbz,ez,gx,gy,gz)
! !$OMP&  hbx,hby,arlp,cax,cay,egz,ex,ey,ecz,egx,egy,concut2,ebx,
! !$OMP& eby,ebz,ecx,ecy,ica,caz,apa,alm,nom,arlm,apm,acops,mv,
! !$OMP& app,alp,ala,noa,arla,apar,nop,ehb1) 
!$OMP target teams loop 
!!$OMP& map(to:sec) !,jbin,seq,x,z,y) !,hbz,ez,gx,gy,gz)
! !$OMP&  hbx,hby,arlp,cax,cay,egz,ex,ey,ecz,egx,egy,concut2,ebx)
! !$OMP& eby,ebz,ecx,ecy,ica,caz,apa,alm,nom,arlm,apm,acops,mv,
! !$OMP& app,alp,ala,noa,arla,apar,nop,ehb1)
!$OMP& map(to:sec,jbin,icnt1,sumct1,ehb5b,ehb5a,
!$OMP& ehb3,ehb2,ehb1c,ehb1b,ehb1a) 
!!!!!!!!!!!!!!!!!!! Correct upto here 
...
! !$acc&    default(present) 
!!$OMP target teams distribute 
! !$OMP&        reduction(+:EHB1,EHB1a,EHB1b,EHB1c,EHB2) 
...
!!!!!!Correct !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!$OMP loop bind(parallel) private(i,noa,nom,nop,fact,axki,ayki,azki,dist,bxi,byi,bzi,
!$OMP&          axi,ayi,azi,idist)
!$OMP&   reduction(ICNT1,SUMCT1,EHB4,EHB1a)
         do 1001 i=1,Lch
            iend=max(k+1,kk)
...
!!$OMP end target data  !! end for target data map region 
!NEW1 !$OMP target exit data map (from:sec,jbin,seq,x,z,y,hbz,ez,gx,gy,gz)
! !$OMP&  hbx,hby,arlp,cax,cay,egz,ex,ey,ecz,egx,egy,concut2,ebx,
! !$OMP& eby,ebz,ecx,ecy,ica,caz,apa,alm,nom,arlm,apm,acops,mv,
! !$OMP& app,alp,ala,noa,arla,apar,nop,ehb1)       
!!!!!!!!!!!!! exit data ends here      
! !$omp end target teams distribute parallel do simd  !!this is correct
!$acc wait(1)
! !$omp end target teams 
! this 1!$acc end data
...
!!$OMP update from(EHB1,EHB1a,EHB1b,EHB1c,EHB2,EHB3,EHB4,
! !$acc&                        EHB5,EHB5a,EHB5b,ICNT1,SUMCT1)
