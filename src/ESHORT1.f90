...
! !$acc data copyin(mcomca(:),n_tem(:)) 
!$acc data present(distl(:,:), kdisl(:,:),mdisl(:) 
...
!$acc parallel loop reduction(+:ESHORT2) default(present) async(1)
      do 1 i=iiii,jjjj
         iseq=seq(i)
         if(mv(i).gt.0)then
            axi=x(i)
...
*********E13 --------------------------------------->
      i1=max(iiii-1,1)
      i2=min(jjjj-1,Lch-2)
!$acc parallel loop reduction(+:ESHORT6) default(present) async(1)
      do i=i1,i2
...
****  Protein-like bias2: b(i), b(i+4) parallel------->
      i1=max(iiii-4,1)
      i2=min(jjjj-1,Lch-5)
!$acc parallel loop reduction(+:ESHORT5) async(1) default(present)
      do i=i1,i2
