...
!$OMP target data map(to:mv,mdisl,seq,x,y,z,hh,ez,gx,gy,egz,ex,ey,ee,
!$OMP& egx,egy,gz,distl,afs,kdisl,ica,dev,area,mdis,kdis,dist) 
!$OMP target  data map(from: dist, dev, kdis)
! !$OMP target data map(from: dist,dev, kdist)
! !$OMP target teams distribute parallel do num_teams(80) nowait  
!!$OMP&  map(tofrom:mv,mdisl,seq,x,y,z,hh,ez,gx,gy,egz,ex,ey,ee,
!!$OMP& egx,egy,gz,distl,afs,kdisl,ica,dev,area,mdis,kdis,dist)
!!$OMP target data map(to:mv,mdisl,seq,x,y,z,hh,ez,gx,gy,egz,ex,ey,ee,
!!$OMP& egx,egy,gz,distl,afs,kdisl,ica,dev,area,mdis,kdis,dist)
! !$OMP target teams distribute parallel do num_teams(80) nowait 
! !$OMP&  map(to:mv,mdisl,seq,x,y,z,hh,ez,gx,gy,egz,ex,ey,ee,
! !$OMP& egx,egy,gz,distl,afs,kdisl,ica,dev,area,mdis,kdis,dist) 
      do 1 i=iiii,jjjj
         iseq=seq(i)
         if(mv(i).gt.0)then
...
! !$OMP target map(to:axi,aax,ayi,aay,azi,aaz)
!$OMP target teams distribute parallel do num_teams(80) nowait
!$OMP& map(tofrom:kdisl,distl)
         do k=1,N_disL
...
! !$OMP end target teams distribute parallel do
!!$OMP end target teams distribute parallel do 
!!$OMP target exit data map(from:mv,mdisl,seq,x,y,z,hh,ez,gx,gy,egz,ex,ey,ee,
!!$OMP& egx,egy,gz,distl,afs,kdisl,ica,dev,area,mdis,kdis,dist)
!$OMP end target data 
! !$OMP& map(tofrom:mv,mdisl,seq,x,y,z,hh,ez,gx,gy,egz,ex,ey,ee,
! !$OMP& egx,egy,gz,distl,afs,kdisl,ica,dev,area,mdis,kdis,dist)
