...
!// !$acc& ,afs(:),noaa(:),nomm(:),envir(:,:,:,:,:),eprofn) !copy(eprofn)
!// !$acc kernels async(2)  
!!$OMP target enter data map(tofrom: nomm(:)) map(tofrom: nop(:))
...
!$OMP&  map(to: nomm, nop, nopp, noaa, nom, noa) private(pp)
! !$acc loop gang(1024)
         do pp=1,Lch
...
! !$OMP target exit data map(delete: nomm(:)) map(delete: nop(:))
! !$OMP    map(delete: nopp(:)) map(delete: noaa(:)) map(delete: nom(:))
! !$OMP    map(delete: noa(:))
!!$OMP target exit data map(delete: nomm(:),nop(:),nopp(:),noaa(:),nom(:),noa(:))
! !$acc end kernels
! !$acc end data
c            energ=energ+de
