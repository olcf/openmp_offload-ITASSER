...
c     backup old path ------------>
 204  oo(m)=ica(m)
      ...
      oz(n1)=z(n1)
!$acc kernels async(2)
! !$OMP target enter data map(to: ex,etz_n,ex_n,ey_n,ey,ez_n,ez,egx,ecz_n,egx_n,egy_n,
! !$OMP& egy,egz,ecx,ebz_n,ecx_n,,ecy_n,ecy,ecz,ebx,ebx_n,eby_n,eby,ebz,etx,
! !$OMP& egz_n,etx_n,ety_n,ety,etz)
!$OMP target teams distribute parallel do num_teams(80) nowait
!!$OMP&  map(tofrom: ex,etz_n,ex_n,ey_n,ey,ez_n,ez,egx,ecz_n,egx_n,egy_n,
!!$OMP& egy,egz,ecx,ebz_n,ecx_n,ecy_n,ecy,ecz,ebx,ebx_n,eby_n,eby,ebz,etx,
!!$OMP& egz_n,etx_n,ety_n,ety,etz)
      do i=m2,n2
         ex_o(i)=ex(i)          !CA
...
!$OMP target teams distribute parallel do num_teams(80) nowait
      ...
      enddo
!$OMP end target teams distribute parallel do 
! !$OMP target exit data map(to: ex,etz_n,ex_n,ey_n,ey,ez_n,ez,egx,ecz_n,egx_n,egy_n,
! !$OMP& egy,egz,ecx,ebz_n,ecx_n,,ecy_n,ecy,ecz,ebx,ebx_n,eby_n,eby,ebz,etx,
! !$OMP& egz_n,etx_n,ety_n,ety,etz)
